...
void native_get(void) {
    ...
    if (IS_LIST(val)) {
        ...
        if (i >= string_len) {
            PRINT_ERR("Index out of bounds: %d for string of length %d.", i, string_len);
            exit(1);
        }
        if (i < 0) {
            i = (i % string_len + string_len) % string_len;
        }
        ObjectString *character = object_string_allocate(&vm.strings, &AS_STRING(val)->chars[i], 1);
...
void native_set(void) {
    ...
    if (IS_LIST(val)) {
        if (i >= AS_LIST(val)->list.count) {
            PRINT_ERR("Index out of bounds: %d for list of size %d.", i, AS_LIST(val)->list.count);
            exit(1);
        }
        push((AS_LIST(val)->list.values[i]));
    }
